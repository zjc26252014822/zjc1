...
static void Rebound(void){id o=activeManager;Setup(o);if(!ready){Log("SKIP not-ready\n");return;}uint8_t*base=(uint8_t*)(__bridge void*)o;double*f=(double*)(base+frameOffset),*p=(double*)(base+dragOffset);double w=f[0],h=f[1],x=p[0],y=p[1],stateX=f[3];if(!isfinite(w)||!isfinite(h)||!isfinite(x)||!isfinite(y)||w<80||h<80){Log("SKIP values f=%.2f %.2f off=%.2f %.2f\n",w,h,x,y);return;}CGRect s=UIScreen.mainScreen.bounds;double cx=(s.size.width-w)*.5,cy=(s.size.height-h)*.5;double loX=Edge-cx,hiX=s.size.width-Edge-w-cx,loY=Top+Edge-cy,hiY=s.size.height-Bottom-Edge-h-cy;double nx=MIN(MAX(x,loX),hiX),ny=MIN(MAX(y,loY),hiY);Log("SETTLE off=(%.2f,%.2f) stateX=%.2f size=(%.2f,%.2f) new=(%.2f,%.2f)\n",x,y,stateX,w,h,nx,ny);if(loX<=hiX&&loY<=hiY&&(nx!=x||ny!=y)){/* Stheno copies finalFrame[3] back into finalOffset.x. Keep both synchronized. */p[0]=nx;p[1]=ny;f[3]=nx;Log("WROTE offset+frameState\n");}else Log("NO-WRITE\n");}
